...
/**
 * Parameters for Wake-on-LAN operation
 */
interface WakeOnLanParams {
    /** UDP port to send the magic packet to (typically 7 or 9) */
    port: number;
    /** Optional hostname to resolve to IP address */
    host?: string;
    /** Optional IP address to send the packet to */
    ip?: string;
    /** Whether to use relay mode (wait for incoming packet before sending) */
    relay?: boolean;
    /** Port to listen on in relay mode */
    relayPort?: number;
}
 ...
 */
function send(mac: string, ip: string, magicPacket: Buffer, params: WakeOnLanParams): void {
    // Create UDP socket (IPv4 or IPv6 based on target IP)
    const socket = createSocket(isIPv6(ip) ? 'udp6' : 'udp4');
 ...
 */
function relay(mac: string, ip: string, magicPacket: Buffer, params: WakeOnLanParams): void {
 ...
 */
function getIP(params: WakeOnLanParams): Promise<string> {
    return new Promise(resolve => {
        if (!params.host) {
            ...
            dnsResolve(params.host, (err, addresses) => {
                if (err) {
                    console.error(err);
                    resolve(BROADCAST);
                } else {
 ...
 * ```
 */
export default function wakeOnLan(mac: string, params: WakeOnLanParams): void {
    void getIP(params).then((ip: string) => {
