...
const pingFallback_1 = require("./lib/pingFallback");
...
class PingAdapter extends adapter_core_1.Adapter {
                ...
                if (!state.val && id.endsWith('.browse.running')) {
                    ...
                    if (task) {
                        if (state.val === false) {
                            ...
                            this.sendWakeOnLan(task).catch(e => this.log.error(`Cannot send WoL to ${task.host}: ${e}`));
                        }
                    }
                }
            },
        ...
        switch (obj.command) {
            case 'ping': {
                ...
                if (obj.callback && obj.message) {
                    const { err, result } = await this.probeWithFallback(obj.message);
            ...
            case 'wakeOnLan': {
                ...
                if (!mac || !(0, wakeOnLan_1.isMACValid)(mac)) {
                ...
                (0, wakeOnLan_1.default)(mac, { port: 9, ip });
    ...
    noteDeniedPing(stderr) {
        if (this.icmpDenied) {
            return;
        }
        this.icmpDenied = true;
        (0, pingFallback_1.deniedHint)(stderr, this.tcpFallbackPorts.length ? this.tcpFallbackPorts : undefined).forEach(line => this.log.warn(line));
    }
    /**
     ...
     */
    probeWithFallback(host, useHping3) {
        return new Promise(resolve => ping.probe(host, { log: this.log.debug, useHping3 }, (err, result) => {
            if (!result?.denied) {
                resolve({ err, result });
                return;
            }
            ...
            (0, pingFallback_1.probeTcp)(host, this.tcpFallbackPorts, TCP_FALLBACK_TIMEOUT, tcp => {
                this.log.debug(`${host} asked over TCP: ${tcp.alive ? `answered on port ${tcp.port}` : 'no answer'}`);
                resolve({
                    err,
    ...
    async pingSingleDevice(task, retryCounter) {
    ...
    async sendWakeOnLan(task) {
        ...
        if (!mac) {
            this.log.warn(`Cannot send Wake-on-LAN to ${task.host}: MAC address unknown. Browse network first or configure MAC in device settings.`);
            return;
        }
    ...
    async main() {
        ...
        this.tcpFallbackPorts = this.config.tcpFallback ? (0, pingFallback_1.parsePorts)(this.config.tcpFallbackPorts) : [];
